...
from scanner_core import adminPanel, crawlerURLs, headers_check, sqlScan, utilities, dnsLookups, logger, hostInfoLookup, dirBuster
...
@click.option('--th', '--threads',
              ...
              metavar='THREADS', default=20, show_default = True)

@click.option('--ports', default=1, help='Choose the ports that you need to scan. USAGE: pyvs --url http://google.com --ports 80 or multiple ports --ports 443, 22, 21', 
              show_default=True, metavar='PORT NUMB')
...
def targetScan(url, st, th, ports, ip, lookup,ftype):
...
#function logic for choosing the scan with the specified parameters
def headercheck(url, wordlist, fsave, redirect):
...
#function logic for choosing the scan with the specified parameters
def sqlscanner(url, wordlist, fsave, redirect):
    
    """
    Use this command to scan a website for sql vulnerabilities
    """
    if url is not None and wordlist is not None:
        sqlScan.startSQL(url,fsave)  
...
#command which is used to scan the website for XSS vulnerabilties
@cli.command()
#arguments for the commands
@click.option('--url', '--u',
              help='When using this option please input the URL you would wish to scan in the format specified:\b [http://google.com , google.com, www.google.com] (DO NOT USE THE EXAMPLE URL SHOWN, FOR IP SCAN USE THE --IP OPTION. For multiple URLS scan please add you URL .txt file to the \\data folder. The URLS should be separated with comma and start on new line or same line comma separated.', 
...
#function logic for choosing the scan with the specified parameters
def xsscaner(url, wordlist, fsave, redirect):
    
    """
    Use this command to scan the website for XSS vulnerabilities
    """
    if url is not None and wordlist is not None:
        sqlScan.startSQL(url,fsave)  